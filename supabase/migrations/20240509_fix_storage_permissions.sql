-- Drop existing storage policies for banner-images bucket
DO $$
DECLARE
  bucket_id TEXT;
BEGIN
  -- Get the bucket ID for banner-images
...
BEGIN
  ...
  ELSE
    -- Drop existing policies for this bucket
    BEGIN
      DROP POLICY IF EXISTS "Allow public read access to " || bucket_name ON storage.objects;
      DROP POLICY IF EXISTS "Allow authenticated users to upload " || bucket_name ON storage.objects;
      DROP POLICY IF EXISTS "Allow authenticated users to update " || bucket_name ON storage.objects;
      DROP POLICY IF EXISTS "Allow authenticated users to delete " || bucket_name ON storage.objects;
    ...
    -- Create new policies with proper permissions
    -- Public read access
    CREATE POLICY "Allow public read access to " || bucket_name
      ON storage.objects FOR SELECT
      USING (bucket_id = bucket_id);
    
    -- Authenticated users can upload
    CREATE POLICY "Allow authenticated users to upload " || bucket_name
      ON storage.objects FOR INSERT
      TO authenticated
      WITH CHECK (bucket_id = bucket_id);
    
    -- Authenticated users can update their own files
    CREATE POLICY "Allow authenticated users to update " || bucket_name
      ON storage.objects FOR UPDATE
      TO authenticated
      USING (bucket_id = bucket_id);
    
    -- Authenticated users can delete their own files
    CREATE POLICY "Allow authenticated users to delete " || bucket_name
      ON storage.objects FOR DELETE
      TO authenticated
      USING (bucket_id = bucket_id);
...
-- Create a function to test bucket permissions
